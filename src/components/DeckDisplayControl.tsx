...
export default function DeckDisplayControl({
  defaultView,
  onClick,
}: DeckDisplayControlProps) {
  const [selected, setSelected] = useState<DeckDisplayControlView>(
    defaultView || "spread"
  );
  ...
  return (
    <div className="deck-display-control">
      ...
      <button
        className="small"
        aria-pressed={selected === "spread"}
        onClick={() => handleClick("spread")}
      >
